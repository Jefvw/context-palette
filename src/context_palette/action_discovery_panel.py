...
ALL_CONTEXTS_FILTER_LABEL = "All contexts"
CONTEXT_FILTER_DISAMBIGUATION_SUFFIX = " — Context"
...
def context_filter_choices(
    contexts: tuple[str, ...],
) -> tuple[tuple[str, ...], dict[str, str], dict[str, str]]:
    """Return unique display labels without changing canonical Context names."""

    used = {ALL_CONTEXTS_FILTER_LABEL.casefold()}
    displays: list[str] = []
    values_by_display: dict[str, str] = {}
    displays_by_value: dict[str, str] = {}
    for context in contexts:
        if context.casefold() == "general":
            continue
        display = context
        while display.casefold() in used:
            display += CONTEXT_FILTER_DISAMBIGUATION_SUFFIX
        used.add(display.casefold())
        displays.append(display)
        values_by_display[display.casefold()] = context
        displays_by_value[context.casefold()] = display
    return tuple(displays), values_by_display, displays_by_value


def context_filter_display_label(
    contexts: tuple[str, ...],
    context: str | None,
) -> str:
    if context is None:
        return ALL_CONTEXTS_FILTER_LABEL
    _choices, _values, displays = context_filter_choices(contexts)
    return displays.get(context.casefold(), context)
...
class ActionDiscoveryPanel:
    ...
    def __init__(
        ...
        *,
        ...
        self.manage_contexts = manage_contexts
        self._context_options: tuple[str, ...] = ()
        self._context_values_by_display: dict[str, str] = {}
        self._context_displays_by_value: dict[str, str] = {}
        ...
        # Tk keyboard traversal follows widget stacking order. The toolbar is
        # constructed early so its controls can be reused across scopes, then
        # raised here so Tab reaches Find, Filter, and results before commands.
        self.tool_rail.lift()
    ...
    def set_filter_indicators(
        ...
        *,
        ...
        active_values = tuple(
            value
            for value in (primary_value, context_value, tag_value, *saved_values)
            if value
        )
    ...
    def _clear_active_filters(self) -> None:
        ...
        if self.context_filter_var.get() != ALL_CONTEXTS_FILTER_LABEL:
    ...
    def set_contexts(self, contexts: tuple[str, ...]) -> None:
        """Set canonical specific Context choices for the shared filter."""
        ...
        (
            ...
            self._context_values_by_display,
            self._context_displays_by_value,
        ) = context_filter_choices(contexts)
    ...
    def _set_scope_options_menu(self, scope: str) -> None:
        ...
        if scope == DISCOVERY_ACTIONS:
            ...
            help_text = "Filter Actions by context, type, or tag."
            ...
            help_text = (
                "Create or update a Work Item, or filter by context, project, or tag."
            )
        else:
        ...
        menu.add_separator()
        menu.add_command(label="Manage contexts…", command=self.manage_contexts)
    ...
    def _queue_context_picker(self) -> None:
        # Let the native menu return before the grabbed popup starts layout.
    ...
    def _show_context_picker(self) -> None:
        ...
        self.context_picker_popup = SearchableSelectionPopup(
            ...
            title="Choose a Context filter",
            empty_label=ALL_CONTEXTS_FILTER_LABEL,
            search_label="Find context",
            item_name="context",
        )

    def _select_context_from_picker(self, selected: tuple[str, ...]) -> None:
        value = selected[0] if selected else ALL_CONTEXTS_FILTER_LABEL
        if value == ALL_CONTEXTS_FILTER_LABEL:
            self.select_context_filter(None)
            return
        canonical = self._context_values_by_display.get(value.casefold(), value)
        self.select_context_filter(canonical)
