...
| Excel CSV automation | Implemented with limitations | Optional separately bootstrapped Python Excel engine; accepts up to 100 closed exact `.xlsx` paths from Input / Output/drop intake; asynchronous describe-plan-review-execute; all used columns; create-only CSV outputs; no source mutation, overwrite, progress, cancellation, live Excel, sequences, or automatic retry after partial/unknown outcomes |
