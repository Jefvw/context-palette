...
1. `Focus: <name>` and the compact Focus-only control.
2. Compact `All / Actions / Work` scope choices.
3. Find and Filter on one line.
4. An explicit removable `Context filter: ...` chip when filtering.
...
Focus and Context filtering remain separate in both state and wording:

- Focus prioritizes real members and supplies slots 6-0; it does not narrow
  global Find.
- A Context filter narrows results; it does not change Focus.
- An empty Focus never gains unrelated green fallback rows, and Focus-only
  shows a truthful empty state.
...
Work Items page, and collapsed pins. Removing the direct destination remains a
separate navigation decision rather than a hidden consequence of this visual
batch.
...
- a compact selected-item strip for tags, Contexts, and folder opening.
...
- collapsed optional pins with literal Show pins / Hide pins controls;
...
- independent, truthful Focus and Context behavior;
...
5. Focus and Context still mean different things without reading Help.
